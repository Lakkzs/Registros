...
module.exports = {
    ...
    rt_altaInfoHobbies: async (req, res) => {
        let body = req.body
        let resultado = (await db.infoColaborador.altaInfoHobbies(body)).datos
        console.log(resultado)
        res.json({ status: 'OK', resultado })
    },
    ...
    rt_cargaInfoHobbies: async (req, res) => {
        try {
            let datos = (req.body)
            let resultado3 = (await db.altas.cargaInfoHobbies(datos)).datos
            console.log(123, resultado3)
            
            if (resultado3[0] != undefined) {
                let hobbies = resultado3[0].HOBBIES
                let pastel = resultado3[0].PASTEL
                let musica = resultado3[0].MUSICA
                console.log(111)
                res.render('partials/hobbies', { datos: resultado3.datos, HOBBIES: hobbies, PASTEL: pastel, MUSICA: musica, name: "txtHobbies", id: "txtHobbies", name2: "txtPastel", id2: "txtPastel", id3: "txtMusica", name3: "txtMusica" }, (error, html) => {
                    res.json({ html })
                })
            } else {
                console.log(222)
                res.render('partials/hobbies', {name: "txtHobbies", id: "txtHobbies", name2: "txtPastel", id2: "txtPastel", id3: "txtMusica", name3: "txtMusica" }, (error, html) => {
                    res.json({ html })
                })
            }
